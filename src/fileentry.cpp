...
namespace zipios {
	 ...
	 */
	FileEntry::FileEntry(FilePath const &filename, std::string const &comment) : m_filename(filename), m_comment(comment) {
	}
	...
	bool FileEntry::isEqual(FileEntry const &file_entry) const {
		return m_filename == file_entry.m_filename && m_comment == file_entry.m_comment && m_uncompressed_size == file_entry.m_uncompressed_size && m_unix_time == file_entry.m_unix_time
			&& m_compress_method == file_entry.m_compress_method && m_crc_32 == file_entry.m_crc_32 && m_has_crc_32 == file_entry.m_has_crc_32 && m_valid == file_entry.m_valid;
	...
	void FileEntry::setMethod(StorageMethod method) {
		switch (method) {
			...
			default: throw InvalidStateException("unknown method");
		}
	...
	std::string FileEntry::toString() const {
		...
		if (isDirectory()) {
			...
			sout << " (" << m_uncompressed_size << " byte" << (m_uncompressed_size == 1 ? "" : "s");
