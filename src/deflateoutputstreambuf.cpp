...
namespace zipios {
	 ...
	 */
	DeflateOutputStreambuf::DeflateOutputStreambuf(std::streambuf *outbuf) : FilterOutputStreambuf(outbuf), m_invec(getBufferSize()), m_outvec(getBufferSize()) {
	...
	bool DeflateOutputStreambuf::init(FileEntry::CompressionLevel compression_level) {
		...
		switch (compression_level) {
			case FileEntry::COMPRESSION_LEVEL_DEFAULT: zlevel = Z_DEFAULT_COMPRESSION; break;

			case FileEntry::COMPRESSION_LEVEL_SMALLEST: zlevel = Z_BEST_COMPRESSION; break;

			case FileEntry::COMPRESSION_LEVEL_FASTEST: zlevel = Z_BEST_SPEED; break;

			case FileEntry::COMPRESSION_LEVEL_NONE: throw std::logic_error("the compression level NONE is not supported in DeflateOutputStreambuf::init()"); // LCOV_EXCL_LINE

			default:
				if (compression_level < FileEntry::COMPRESSION_LEVEL_MINIMUM || compression_level > FileEntry::COMPRESSION_LEVEL_MAXIMUM) {
					// This is excluded from the coverage since if we reach this
					// line there is an internal error that needs to be fixed.
