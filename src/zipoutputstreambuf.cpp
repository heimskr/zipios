...
namespace zipios {
	 ...
	 */
	ZipOutputStreambuf::ZipOutputStreambuf(std::streambuf *outbuf) : DeflateOutputStreambuf(outbuf) {
	}
	...
	void ZipOutputStreambuf::putNextEntry(FileEntry::pointer_t entry) {
		...
		m_overflown_bytes = 0;
		switch (m_compression_level) {
			case FileEntry::COMPRESSION_LEVEL_NONE: setp(&m_invec[0], &m_invec[0] + getBufferSize()); break;

			default: init(m_compression_level); break;
		}
	...
	int ZipOutputStreambuf::overflow(int c) {
		...
		switch (m_compression_level) {
			...
			default: return DeflateOutputStreambuf::overflow(c);
		}
	}
