...
namespace zipios {
	 ...
	 */
	ZipCentralDirectoryEntry::ZipCentralDirectoryEntry(FileEntry const &entry) : ZipLocalEntry(entry) {
	}
	...
	size_t ZipCentralDirectoryEntry::getHeaderSize() const {
		...
		return 46 /* sizeof(ZipCentralDirectoryEntryHeader) */
			+ m_filename.length() + (m_is_directory ? 1 : 0) + m_extra_field.size() + m_comment.length();
	}
	...
	void ZipCentralDirectoryEntry::write(std::ostream &os) {
		 ...
		 */
		if (m_filename.length() > 0x10000 || m_extra_field.size() > 0x10000 || m_comment.length() > 0x10000) {
...
// Solaris defines _ILP32 for 32 bit platforms
		...
		if (m_compressed_size >= 0x100000000ULL || m_uncompressed_size >= 0x100000000ULL || m_entry_offset >= 0x100000000LL) {
