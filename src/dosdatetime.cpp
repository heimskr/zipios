...
namespace zipios {
	...
	bool DOSDateTime::isValid() const {
		...
		return conv.m_fields.m_second < 30 // remember we only keep `sec / 2` in a DOS time field
			&& conv.m_fields.m_minute < 60 && conv.m_fields.m_hour < 24 && conv.m_fields.m_mday > 0 && conv.m_fields.m_mday <= daysInMonth() && conv.m_fields.m_month > 0 && conv.m_fields.m_month < 13;
	}
