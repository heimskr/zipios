...
namespace zipios {
	 ...
	 *                       sub-directories.
	 */
	DirectoryCollection::DirectoryCollection(std::string const &path, bool recursive) : m_recursive(recursive), m_filepath(path) {
...
#else
		struct read_dir_t {
			read_dir_t(FilePath const &path) : m_dir(opendir(static_cast<std::string>(path).c_str())) {
				if (m_dir == nullptr) {
