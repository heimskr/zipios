...
namespace zipios {
	 ...
	 */
	ZipEndOfCentralDirectory::ZipEndOfCentralDirectory(std::string const &zip_comment) : m_zip_comment(zip_comment) {
	}
...
// Solaris defines _ILP32 for 32 bit platforms
		...
		if (m_central_directory_size >= 0x100000000UL || m_central_directory_offset >= 0x100000000L) {
