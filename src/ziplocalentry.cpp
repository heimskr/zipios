...
namespace zipios {
	 ...
	 */
	ZipLocalEntry::ZipLocalEntry() : FileEntry(FilePath("")) {
	}
	 ...
	 */
	ZipLocalEntry::ZipLocalEntry(FileEntry const &src) : FileEntry(src), m_is_directory(src.isDirectory()) {
	}
	...
	bool ZipLocalEntry::isEqual(FileEntry const &file_entry) const {
		...
		return FileEntry::isEqual(file_entry) && m_extract_version == ze->m_extract_version && m_general_purpose_bitfield == ze->m_general_purpose_bitfield && m_is_directory == ze->m_is_directory;
	...
	size_t ZipLocalEntry::getHeaderSize() const {
		...
		return 30 /* sizeof(ZipLocalEntryHeader) */
			+ m_filename.length() + (m_is_directory ? 1 : 0) + m_extra_field.size();
	}
	...
	void ZipLocalEntry::write(std::ostream &os) {
		if (m_filename.length() > 0x10000 || m_extra_field.size() > 0x10000) {
		...
		if (m_compressed_size >= 0x100000000UL || m_uncompressed_size >= 0x100000000UL) {
