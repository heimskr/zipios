...
namespace zipios {
	 ...
	 */
	DirectoryEntry::DirectoryEntry(FilePath const &filename, std::string const &comment) : FileEntry(filename, comment) {
	...
	uint32_t DirectoryEntry::computeCRC32() const {
		...
		if (!m_filename.isDirectory()) {
			...
			in.open(m_filename);
			if (!in.is_open()) {
				throw IOException("Can't open file \"" + m_filename.filename() + "\".");
			}
