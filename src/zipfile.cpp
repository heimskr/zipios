...
namespace zipios {
	 ...
	 */
	ZipFile::ZipFile(std::string const &filename, offset_t s_off, offset_t e_off) : FileCollection(filename), m_vs(s_off, e_off) {
	 ...
	 *                   offset goes toward the beginning of the file.
	 */
	ZipFile::ZipFile(std::istream &is, offset_t s_off, offset_t e_off) : m_vs(s_off, e_off) {
		init(is);
	}
	...
	void ZipFile::saveCollectionToArchive(std::ostream &os, FileCollection &collection, std::string const &zip_comment) {
		try {
			...
			for (auto it(entries.begin()); it != entries.end(); ++it) {
				...
				// not an empty file
				//
				if (!(*it)->isDirectory() && (*it)->getSize() > 0) {
					// get an InputStream
					//
