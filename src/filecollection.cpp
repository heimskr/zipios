...
namespace zipios {
	...
	namespace {
		...
		public:
			 ...
			 */
			explicit MatchName(std::string const &name) : m_name(name) {
			}
		...
		public:
			 ...
			 * \param[in] name  The name of the file being searched.
			 */
			explicit MatchFileName(std::string const &name) : m_name(name) {
			}
	 ...
	 */
	FileCollection::FileCollection(std::string const &filename) : m_filename(filename.empty() ? g_default_filename : filename) {
	}
	 ...
	 */
	FileCollection::FileCollection(FileCollection const &rhs) : m_filename(rhs.m_filename), m_valid(rhs.m_valid) {
	 ...
	 * \sa setLevel()
	 */
	void FileCollection::setMethod(std::size_t limit, StorageMethod small_storage_method, StorageMethod large_storage_method) {
		// make sure the entries were loaded if necessary
		entries();
	 ...
	 * \sa setMethod()
	 */
	void FileCollection::setLevel(std::size_t limit, FileEntry::CompressionLevel small_compression_level, FileEntry::CompressionLevel large_compression_level) {
		// make sure the entries were loaded if necessary
		entries();
