...
namespace zipios {
	 ...
	 */
	GZIPOutputStreambuf::GZIPOutputStreambuf(std::streambuf *outbuf, FileEntry::CompressionLevel compression_level) : DeflateOutputStreambuf(outbuf) {
	...
	void GZIPOutputStreambuf::writeHeader() {
		unsigned char const flg((m_filename.empty() ? 0x00 : 0x08) | (m_comment.empty() ? 0x00 : 0x10));
