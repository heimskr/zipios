...
namespace zipios {
	 ...
	 */
	GZIPOutputStream::GZIPOutputStream(std::ostream &os, FileEntry::CompressionLevel compression_level) : m_ozf(std::make_unique<GZIPOutputStreambuf>(os.rdbuf(), compression_level)) {
		init(m_ozf.get());
	}
	...
	GZIPOutputStream::GZIPOutputStream(std::string const &filename, FileEntry::CompressionLevel compression_level) :
		std::ostream(0), m_ofs(std::make_unique<std::ofstream>(filename.c_str(), std::ios::out | std::ios::binary)), m_ozf(std::make_unique<GZIPOutputStreambuf>(m_ofs->rdbuf(), compression_level)) {
		init(m_ozf.get());
	}
