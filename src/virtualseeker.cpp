...
namespace zipios {
	 ...
	 */
	VirtualSeeker::VirtualSeeker(offset_t start_offset, offset_t end_offset) : m_start_offset(start_offset), m_end_offset(end_offset) {
		if (m_start_offset < 0 || m_end_offset < 0) {
	...
	void VirtualSeeker::vseekg(std::istream &is, offset_t offset, std::ios::seekdir sd) const {
		switch (sd) {
			case std::ios::cur: break;

			case std::ios::beg: offset += m_start_offset; break;
			...
			default: throw std::logic_error("VirtualSeekManager::vseekg(): error - unknown seekdir");
		}
